...
async function loadTeacherDashboard() {
  ...
  try {
    ...
    if (ctx && ctx.myClass) {
      ...
      const studentSnap = await getDocs(query(collection(db, "students"), where("class", "==", ctx.myClass.id)));
      setText("dashStudentCount", studentSnap.size);
...
async function loadMyStudents() {
  ...
  const snap = await getDocs(query(collection(db, "students"), where("class", "==", ctx.myClass.id)));
